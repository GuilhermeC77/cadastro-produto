...
function createDataset(fields, constraints, sortFields) {
  log.info("(GUILHERME) Executando dataset dsCatalogoProd");
  ...
  try {
    ...
    try {
      log.info("(dsCatalogoProd) Antes de realizar a chamada ao REST fluig");
      log.info("(dsCatalogoProd) Parametros da chamada ao REST fluig");
      log.dir(data);
      var restCall = clientService.invoke(JSON.stringify(data));
      log.info("(dsCatalogoProd) Depois da chamada ao REST fluig");
      log.dir(restCall);
      if (restCall.result == "") return datasetError("(dsCatalogoProd) REST retornou vazio.");
      else if (restCall.httpStatusResult != 200) return datasetError(restCall.result);
      var result = JSON.parse(restCall.result);
      result = result.catalogo;
      log.info("(dsCatalogoProd) Chamada realizada com sucesso!");
      log.info("(dsCatalogoProd) Retorno chamada REST");
      log.dir(result);
      for (var i = 0; i < result.length; i++) {
        dataset.addRow([false, result[i].codigo, result[i].descricao]);
      }
    } catch (e) {
      log.info("GUILHERME-log | Erro na chamada do DataSet dsCatalogoProd.js");
      log.dir(e);
